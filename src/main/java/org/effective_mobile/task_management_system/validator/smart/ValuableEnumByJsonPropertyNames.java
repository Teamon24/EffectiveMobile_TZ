...
public interface ValuableEnumByJsonPropertyNames {
    Map<String, Class<? extends ValuableEnum<String>>> map = new HashMap<>() {{
       put(JsonPojos.Task.Field.PRIORITY, Priority.class);
       put(JsonPojos.Task.Field.STATUS, Status.class);
    }};

    static Class<? extends ValuableEnum<String>> get(String fieldName) {
        Class<? extends ValuableEnum<String>> enumClass = map.get(fieldName);
        if (enumClass == null) {
